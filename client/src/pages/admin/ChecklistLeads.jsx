...
export default function ChecklistLeads() {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const res = await fetch(`/api/checklist-leads/${id}/status`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-slate-900">Document Checklist Leads</h1>
      ...
      {selectedLead && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl relative">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-1">Checklist Request Details</h3>
            <p className="text-slate-500 text-sm mb-6">Submitted on {selectedLead.createdAt.toLocaleString()}</p>
            
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  <span className="block text-xs text-slate-500 uppercase font-semibold mb-1">Name</span>
                  <span className="text-slate-900 font-medium">{selectedLead.name}</span>
                </div>
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                ...
                {selectedLead.phone && (
                  <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 col-span-2">
                    <span className="block text-xs text-slate-500 uppercase font-semibold mb-1">Phone</span>
                    <span className="text-slate-900 font-medium">{selectedLead.phone}</span>
                  </div>
                )}
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  <span className="block text-xs text-slate-500 uppercase font-semibold mb-1">Service</span>
                  <span className="text-slate-900 font-medium">{selectedLead.service}</span>
                </div>
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  <span className="block text-xs text-slate-500 uppercase font-semibold mb-1">Country</span>
                  <span className="text-slate-900 font-medium">{selectedLead.country}</span>
                </div>
              </div>
            </div>
